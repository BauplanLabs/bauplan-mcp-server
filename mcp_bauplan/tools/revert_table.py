...
def register_revert_table_tool(mcp: FastMCP) -> None:
    ...
    async def revert_table(
        ...
        try:
            ...
            if ctx:
                await ctx.info(f"Reverting table '{table}' from source ref '{source_ref}' into branch '{into_branch}'")
            ...
            logger.info(f"Successfully reverted table '{table}' from '{source_ref}' into branch '{into_branch}'")
