...
def register_get_job_logs_tool(mcp: FastMCP) -> None:
    ...
    async def get_job_logs(
        job_id_prefix: str, api_key: Optional[str] = None, ctx: Context = None
    ) -> JobLogsList:
        """
        Get job logs by job ID prefix.
        ...
        try:
            ...
            for log in logs_result:
                job_log_info = JobLogInfo(message=log.message, stream=str(log.stream))
                job_log_info_list.append(job_log_info)

            # Log successful retrieval
            logger.info(
                f"Successfully retrieved {len(job_log_info_list)} job logs for prefix: {job_id_prefix}"
            )

            return JobLogsList(
                logs=job_log_info_list, total_count=len(job_log_info_list)
            )
            ...
            error_msg = str(e)
            logger.error(
                f"Error getting job logs for prefix {job_id_prefix}: {error_msg}"
            )
            raise ToolError(
                f"Failed to get job logs for prefix {job_id_prefix}: {error_msg}"
            )
