...
def register_create_namespace_tool(mcp: FastMCP) -> None:
    ...
    async def create_namespace(
        ...
        branch: Annotated[
            str,
            Field(
                description="Writable branch where the namespace will be created.",
            ),
        ],
        ...
        """
        Create a namespace on a writable Bauplan branch.
        Use this before creating or importing tables into a namespace that does not exist yet.
        """
        ...
        try:
            ...
            raise ToolError(
                f"Error executing create_namespace '{namespace}' in branch '{branch}': {e}"
            ) from e
